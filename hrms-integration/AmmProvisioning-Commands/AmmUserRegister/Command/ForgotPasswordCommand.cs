...
public class ForgotPasswordCommand : IRequest<ResultNew<bool>>
{
    public string EmailAddress { get; set; }
 
}

public class ForgotPasswordCommandCommandHandler : IRequestHandler<ForgotPasswordCommand, ResultNew<bool>>
{
    ...
    public ForgotPasswordCommandCommandHandler(IAmmDbContext context,
    ...
    public async Task<ResultNew<bool>> Handle(ForgotPasswordCommand request, CancellationToken cancellationToken)
    {
        ...
        var empCode = user.USER_EMP_CODE; 
        ...
        {
            COMP_CODE = user.AUD_COMP_CODE,
            MAIL_TO = request.EmailAddress,
            MAIL_SUBJECT = "Your OTP for Password Reset",
            BODY_HTML = $"<p>Dear {user.USER_SHORT_NAME},</p><p><br> Your OTP is <strong>{otp}</strong>. It is valid for 10 minutes.</p>",
             ENABLED_YN = "Y"
              , 
        };
